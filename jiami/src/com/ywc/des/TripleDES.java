...
/**
 * 文件加密 加密方法：三重des加密 加密过程：对选中的文件加密后在同文件夹下生成一个增加了".tdes" 扩展名的加密文件
 * 解密过程：对选中的加密文件（必须有".tdes"扩展名）进行解密
 */
public class TripleDES {

	// public static void main(String args[]) {
	// TripleDES da = new TripleDES();
	...
	// String name = Constant.FILES_FOLDER_PATH + scan.next();// 文件名
	...
	public void encrypt(File fileIn, String sKey) {
		try {
			if (sKey.length() == 48) {
				...
				String fileOut = Constant.CIPHERTEXT_PATH + "3DES_" + fileIn.getName();
	...
	public void decrypt(File fileIn, String sKey) {
		try {
			if (sKey.length() == 48) {
				String strPath = Constant.PLAINTEXT_PATH + fileIn.getName().replaceAll("3DES_", "");
	...
	public void encrypt() {
		...
		da.encrypt(new File(Constant.PLAINTEXT_PATH + name), da.md5s(pass1) + da.md5s(pass2) + da.md5s(pass3));
	...
	public void decrypt() {
		...
		System.out.println("请输入文件名:\n");
		String name = scan.next();// 文件名
		da.decrypt(new File(Constant.CIPHERTEXT_PATH + name), da.md5s(pass1) + da.md5s(pass2) + da.md5s(pass3));
